error id: file:///E:/毕设/spingboot/src/main/java/com/example/service/ViolationService.java:_empty_/violationMapper#selectByOnweridCountSQU#
file:///E:/毕设/spingboot/src/main/java/com/example/service/ViolationService.java
empty definition using pc, found symbol in pc: _empty_/violationMapper#selectByOnweridCountSQU#
...
public void userHandleViolation1(Violation violation) {
    // 1. 根据违章单中的车主身份证和车牌号，查询对应的用户与车辆信息
    User user = userMapper.selectByIdCar(violation.getOwnerId());
    Car car = carMapper.selectByPlate(violation.getPlate());
    // 2. 根据违章代码查询该违章对应的扣分规则
    ViolationId violationId = violationIdMapper.selectAllById(violation.getViolationid());
    // 3. 计算用户支付违章后的剩余驾驶分，并保证分数不会小于 0
    int points = user.getPoints() - violationId.getPoints();
    user.setPoints(Math.max(points, 0));
    // 4. 若剩余驾驶分小于等于 0，则将用户状态设置为封禁
    if (points <= 0) {
        user.setStatus("OUT");
    } else {
        // 5. 若该车辆仍存在其他未处理违章，则车辆保持异常状态，否则恢复为正常状态
        car.setStatus(violationMapper.selectByPlateCountSQU(violation.getPlate()) > 1 ? "SQU" : "NOR");
        // 6. 若该用户名下不存在其他未处理违章，则将用户状态恢复为正常
        if (violationMapper.@@selectByOnweridCountSQU(violation.getOwnerId()) == 1) {
            user.setStatus("NOR");
        }
    }
    // 7. 将最新的用户状态和车辆状态写回数据库
    userMapper.updateById(user);
    carMapper.updateById(car);
    // 8. 将当前违章单状态更新为已支付
    violation.setStatus("HPD");
