...
<template>
  <div class="dashboard">
    ...
    <div class="quick-actions">
      ...
      <el-row :gutter="30">
          <el-col :span="6">
            <el-card shadow="hover" class="action-card" @click="navigateTo('/user/MyViolation')" :body-style="{ padding: '20px' }">
              <div style="display: flex; align-items: center; justify-content: space-between;">
                <div>
                  <div style="font-size: 14px; color: #6b7280; margin-bottom: 4px;">我的违章数量</div>
          ...
          <el-col :span="6">
            <el-card shadow="hover" class="action-card" @click="navigateTo('/user/myVehicles')" :body-style="{ padding: '20px' }">
              <div style="display: flex; align-items: center; justify-content: space-between;">
                <div>
                  <div style="font-size: 14px; color: #6b7280; margin-bottom: 4px;">我的车辆数量</div>
                  <div style="font-size: 36px; font-weight: 700; color: #2563eb; line-height: 1;">
                    {{ data.carCount }}
                  </div>
                </div>
                <div style="width: 48px; height: 48px; border-radius: 12px; background: #eff6ff; display: flex; align-items: center; justify-content: center;">
                  <el-icon :size="24" color="#2563eb"><Van /></el-icon>
                </div>
              </div>
            </el-card>
          </el-col>
          <el-col :span="6">
            <el-card shadow="hover" class="action-card" :body-style="{ padding: '20px' }" :style="{ backgroundColor: getPointsColor(data.user.points), border: 'none' }">
              <div style="display: flex; align-items: center; justify-content: space-between;">
                <div>
                  <div style="font-size: 14px; color: rgba(255, 255, 255, 0.9); margin-bottom: 4px;">当前驾驶分</div>
                  <div style="font-size: 36px; font-weight: 700; line-height: 1; color: white;">
                    {{ data.user.points === undefined ? 12 : data.user.points }}
                  </div>
                </div>
    ...
    <div class="quick-actions">
      ...
      <el-row :gutter="30">
        <el-col :span="6">
                     <el-card shadow="hover" class="action-card" @click="navigateTo('/user/road')">
             <div class="action-content">
               <el-icon :size="32" color="#059669"><DataLine /></el-icon>
               <h4>实时路况数据</h4>
               <p>查看路况数据</p>
             </div>
           </el-card>
        ...
        <el-col :span="6">
          <el-card shadow="hover" class="action-card" @click="navigateTo('/user/myVehicles')">
            <div class="action-content">
              <el-icon :size="32" color="#1e40af"><Tickets /></el-icon>
              <h4>查看车辆</h4>
              <p>查看车辆数据</p>
            </div>
          </el-card>
        ...
        <el-col :span="6">
          <el-card shadow="hover" class="action-card" @click="navigateTo('/user/MyViolation')">
            <div class="action-content">
        ...
        <el-col :span="6">
          <el-card shadow="hover" class="action-card" @click="navigateTo('/user/UserProfile')">
            <div class="action-content">
...
import { Monitor, VideoCamera, Warning, User, InfoFilled, CircleCheckFilled, TrendCharts, Cpu, Clock, FolderOpened, Setting, DataLine, Tickets, Van, Trophy } from '@element-plus/icons-vue'
...
const getPointsColor = (points) => {
  const p = points === undefined ? 12 : points // 默认为12分
  if (p >= 8) return '#16a34a' // Green
  if (p >= 4) return '#ea580c' // Orange
  return '#dc2626' // Red
}

const getPointsBgColor = (points) => {
  const p = points === undefined ? 12 : points
  if (p >= 8) return '#dcfce7' // Light Green
  if (p >= 4) return '#ffedd5' // Light Orange
...
const loadViolationCount = () => {
  request.get('/user/selectViolationPage', {
    ...
    if (res.code === '200') {
      data.violationCount = res.data.total
    }
  })
}
