  ...
  <div>
    ...
    <div class="card" style="margin-bottom: 5px">
      <el-table :data="data.tableData" stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column label="警号" prop="poid" />
...
<!--        上传用户头像-->
        ...
        <el-form-item label="警号" label-width="80px" prop="poid" >
          <el-input v-model="data.from.poid" autocomplete="off" placeholder="警号"/>
        </el-form-item>
...
const add = () => {
  request.post('/police/add', data.from).then(res => {
    if (res.code === '200') {
...
const update = () => {
  request.put('/police/update', data.from).then(res => {
    if (res.code === '200') {
      ElMessage.success("操作成功")
      load()
    } else {
...
const del = (id) => {
  ElMessageBox.confirm("删除数据后无法恢复，您确认删除吗","删除确认",{ type: 'warning'}).then(()=>{
    request.delete('/police/deleteById/'+id).then(res => {
      if (res.code === '200') {
        ElMessage.success("操作成功")
        load()
      } else {
...
const delBatch = () => {
  ...
  ElMessageBox.confirm("删除数据后无法恢复，您确认删除吗","删除确认",{ type: 'warning'}).then(()=>{
    request.delete('/police/deleteBatch', {data: data.ids}).then(res => {
      if (res.code === '200') {
        ElMessage.success("操作成功")
        load()
      } else {
